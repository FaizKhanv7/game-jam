...
func _input(event):
	if event.is_action_pressed("attack") and not attacking:
		perform_attack()
	if event.is_action_pressed("super") and not attacking:
		perform_super()
...
func perform_attack():
	...
	var default = $AttackArea/CollisionShape2D


	attack_timer.start(0.1)
	await attack_timer.timeout
	default.disabled = true    # disable after duration
	...
	default.disabled = false   # enable only for super
	attacking = false

func perform_super():
	Global.supering = true
	attack_area.monitoring = true
	attacking = true
	sprite_2d.play("super")
	game_manager.decrease_health(2)

	var super_hitbox = $AttackArea/CollisionShape2D2
	
	super_hitbox.disabled = false
	...
	attack_area.monitoring = false
	super_hitbox.disabled = true
	attacking = false
	Global.supering = false
